...
namespace Kcc.Base
{
    public class PlanetManager : MonoBehaviour, IKccMover
    {
        ...
        public void UpdateMovement(out Vector3 goalPosition, out Quaternion goalRotation, float deltaTime)
        {
            ...
            {
                cc.Config.Gravity = (PlanetMover.transform.position - cc.transform.position).normalized * GravityStrength;
            }
        }

        void ControlGravity(CharacterController cc)
        {
            _savedGravity = cc.Config.Gravity;
            _characterControllersOnPlanet.Add(cc);
        }

        void UnControlGravity(CharacterController cc)
        {
            cc.Config.Gravity = _savedGravity;
